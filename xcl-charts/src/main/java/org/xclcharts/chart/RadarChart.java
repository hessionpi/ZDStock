...
public class RadarChart extends RdChart {
     ...
     * 分类轴的数据源
     *
     * @param bitmapSeries 标签集
     */
    public void setCategoriesBitmap(List<Bitmap> bitmapSeries) {
        if (null != categoryAxis) categoryAxis.setBitmapBuilding(bitmapSeries);
    }
        ...
        for (int i = 0; i < dataAxisTickCount; i++) {
            for (int j = 0; j < labelsCount; j++) {
                ...
                if (i == dataAxisTickCount - 1) {
                    ...
                    Bitmap bitmap = categoryAxis.getBitmapSet().get(j);
                    canvas.drawText(label,
                            mArrayLabelX[i][j], mArrayLabelY[i][j], getLabelPaint());
                    if (bitmap != null) {
                           // canvas.drawBitmap(bitmap, mArrayLabelX[i][j] - DrawHelper.getInstance().getTextWidth(getLabelPaint(), label+"》》") / 2, mArrayLabelY[i][j]- DrawHelper.getInstance().getTextWidth(getLabelPaint(), label+"》》") / 4, getLabelPaint());

                        if (j==1){
                            canvas.drawBitmap(bitmap, mArrayLabelX[i][j]- DensityUtil.dip2px(20) , mArrayLabelY[i][j]- bitmap.getWidth()/6, getLabelPaint());
                        }else if(j==4){
                            canvas.drawBitmap(bitmap, mArrayLabelX[i][j] -bitmap.getWidth()/2- DensityUtil.dip2px(20), mArrayLabelY[i][j]- bitmap.getWidth()/6, getLabelPaint());

                        }else{
                            canvas.drawBitmap(bitmap, mArrayLabelX[i][j] -bitmap.getWidth()/2, mArrayLabelY[i][j]- bitmap.getWidth()/6, getLabelPaint());
                        }
                    }


                    //setLabelLocation( mArrayLabelX[i][j], mArrayLabelY[i][j]);
                        /*savePointRecord(tdataID,tchildID, mArrayLabelX[i][j],mArrayLabelY[i][j],
                                mArrayLabelX[i][j]-DrawHelper.getInstance().getTextWidth(getLabelPaint(),label)  ,mArrayLabelY[i][j] -DrawHelper.getInstance().getPaintFontHeight(getLabelPaint())  ,
								mArrayLabelX[i][j]+DrawHelper.getInstance().getTextWidth(getLabelPaint(),label) ,mArrayLabelY[i][j]+DrawHelper.getInstance().getPaintFontHeight(getLabelPaint()));
*/
    ...
    private void calcAllPoints() {
        ...
        labelRadius = this.getRadius() + labelHeight + mLabelOffset;
    ...
    private void drawDataPath(Canvas canvas,
        ...
        for (int p = 0; p < arrayDataX.length; p++) {
            ...
            for (int i = 0; i < dataAxisTickCount; i++) {
                ...
                if (i == dataAxisTickCount - 1) {
                    //绘制最外围的标签
                    String label = categoryAxis.getDataSet().get(p);
                    savePointRecord(dataID, p, mArrayLabelX[i][p], mArrayLabelY[i][p],
                            mArrayLabelX[i][p] - DrawHelper.getInstance().getTextWidth(getLabelPaint(), label), mArrayLabelY[i][p] - DrawHelper.getInstance().getPaintFontHeight(getLabelPaint()),
                            mArrayLabelX[i][p] + DrawHelper.getInstance().getTextWidth(getLabelPaint(), label), mArrayLabelY[i][p] + DrawHelper.getInstance().getPaintFontHeight(getLabelPaint()));
                    Log.d("ZSK", dataID + "++++++++++++++++++++++++" + p);
                }
