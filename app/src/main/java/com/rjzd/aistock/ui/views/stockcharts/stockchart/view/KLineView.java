...
import com.rjzd.aistock.ui.views.stockcharts.stockchart.bean.CrossBean;
...
import com.rjzd.aistock.ui.views.stockcharts.stockchart.utils.IndexParseUtil;
import com.rjzd.aistock.ui.views.stockcharts.stockchart.utils.LineUtil;
import com.rjzd.aistock.ui.views.stockcharts.stockchart.utils.NumberUtil;
...
public class KLineView extends ChartView implements ChartConstant {
    ...
    //烛形图和右侧空白的宽度
    public float DEFUALT_WIDTH = PixelUtil.dp2px(5);
    //K线所有数据
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        //   LogUtil.e("ZSJ", "______________________________" + event.getY());
    ...
    protected void drawText(Canvas canvas) {
        ...
        if (showList.size() <= drawCount) {
            ...
            int minmWidth = (int) ((showList.size() * 1.0) / (drawCount * 1.0) * mWidth);
            DrawUtils.drawKLineXTime(canvas, showList.get(0).getTime(), showList.get(showList.size() - 1).getTime(), minmWidth, mWidth, mainH);

        } else {
            DrawUtils.drawKLineXTime(canvas, showList.get(0).getTime(), null, mWidth, mWidth, mainH);
    ...
    protected void drawVOL(Canvas canvas) {
       ...
       // for (StickData data : showList) {
        for (StickData stickData : data) {
            max = stickData.getCount() > max ? stickData.getCount() : max;
        }
        //如果量全为0，则不画
        if (max == 0) return;
        //2,画量线，多条竖直线
  ...
  */
    ...
    private float parseNumber(double input) {
        float y=(float) ((input - yMin) / yUnit);
        if (y < mainH) {
            return mainH-y ;
        } else {
            return 0;
