...
public class LineUtil {
    ...
    public static ArrayList<CMinute> getAllFenshiData(FenshiDataResponse response) {
            ...
            if (i == 0) {
                ...
                if (morningStopMin == 0 || currentMin <= morningStopMin || currentMin >= afternoonStartMin) {
                    for (int j = 0; j < div; j++) {
                       ...
                       // temp.setTime(beforeObject.getTime() + 5);
                        temp.setCount(0);
                        temp.setMoney(0);
        ...
        if (until <= (hasNight ? nightStopMin : afternoonStopMin) && fsDatasTemp.size() < 14400) {
            ...
            if (div >= 1) {
                for (int j = 0; j <= div; j++) {
                    ...
                    temp.setTime((lasteObject.getTime() + 5));
    ...
    public static double[] getMaxAndMinByYd(double max, double min, double yd) {
        ...
        result[0] = yd + limit+0.02;
        result[1] = yd - limit- 0.02;
