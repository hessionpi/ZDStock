...
public class FenshiView extends ChartView {
    ...
    //分时线昨收
    private double yd;
    boolean isDrawAverageLine = true;
    //是否白盘夜盘一起显示
     ...
     * @param data
     */
    public void setDataAndInvalidate(FenshiDataResponse data,boolean isDrawAverageLine) {
        this.data = data;
        minutes = LineUtil.getAllFenshiData(data);
        setDrawAverageLine(isDrawAverageLine);
    ...
    void drawPrevClose(Canvas canvas) {
        ...
        p.setPathEffect(new DashPathEffect(new float[]{8, 10}, 0));
        //中间实线(横)
        canvas.drawLine(0, mainH * 2 / 4, mWidth, mainH * 2 / 4, p);
       // canvas.drawLine(0, mainH * 2 / 4, mWidth, mainH * 2 / 4, p);
    }
