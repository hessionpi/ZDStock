...
public class LineChartRenderer extends LineRadarRenderer {
    ...
    private void generateFilledPath(final ILineDataSet dataSet,final int startIndex, final int endIndex, final Path outputPath) {
        ...
        final int   MyIndex = (int)dataSet.getFillFormatter().getFillStartIndex(dataSet, mChart);

        final Path filled = outputPath;
        filled.reset();
        // create a new path

        for (int x = startIndex; x <=MyIndex; x++) {
            if (x<MyIndex){
                filled.lineTo(x,fillMin);
            }else{
               // filled.lineTo(x,fillMin);
                /*filled.lineTo(x,fillMin*2);
                filled.lineTo(x,fillMin/2);
                filled.lineTo(x,fillMin*3);
                filled.lineTo(x,fillMin*4);
                filled.lineTo(x,fillMin/3);
                filled.lineTo(x,fillMin/4);*/
                filled.lineTo(x,fillMin*5);
                filled.lineTo(x,fillMin/5);
        ...
        final Entry entry2 = dataSet.getEntryForIndex(MyIndex);
        filled.moveTo(entry2.getX(), fillMin);
        filled.lineTo(entry2.getX(), entry2.getY() * phaseY);
        for (int x = MyIndex+1 ; x <= endIndex; x++) {
